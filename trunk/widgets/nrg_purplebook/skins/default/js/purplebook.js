...
function pb_modify_name(obj) {
    ...
    jQuery(name).focusout(function() {
        ...
        if (jQuery(this).val() != $nodeName.text()) {
            exec_xml('mobilemessage', 'procMobilemessagePurplebookUpdateName', params, function() { $nodeName.text(params['name']); });
        }
        jQuery(this).remove();
    });
    $li.append(name);
...
function pb_modify_phone(obj) {
    ...
    jQuery(phonenum).focusout(function() {
        ...
        if ($this.val() != $nodePhone.text()) {
            exec_xml('mobilemessage', 'procMobilemessagePurplebookUpdatePhone', params, function() { $nodePhone.text(getDashTel(params['phone_num'])); $this.remove(); });
        } else {
            $this.remove();
        }
    });
...
function pb_view_properties(obj) {
    ...
    exec_xml('mobilemessage', 'getMobilemessagePurplebookProperties', params, function(ret_obj,response_tags) { completeGetProperties(obj,ret_obj,response_tags); }, response_tags);
}
...
function pb_share_folder(obj) {
    ...
    params['node_id'] = $node.attr('node_id');
    var response_tags = new Array('error','message','data');
    exec_xml('mobilemessage', 'getMobilemessagePurplebookSharedUsers', params, function(ret_obj,response_tags) { completeGetSharedUsers(obj,ret_obj,response_tags) }, response_tags);
}
...
function clearTrash() {
    ...
    exec_xml('mobilemessage', 'procMobilemessagePurplebookDeleteNode', params, function() {
            /*
...
function updateResultStatus() {
    ...
    jQuery.ajax({
        ...
        , data : { 
                    ...
                    , act : "getMobilemessageListByMessageId"
...
function completeGetPointInfo(ret_obj, response_tags) {
    ...
    if (!g_sms_point || !g_lms_point || !g_mms_point) {
        alert('위젯설정에서 포인트 차감 사용으로 되어 있으나 차감할 포인트가 설정되어있지 않습니다.');
        return false;
    }

    sms_avail = calc_sms(obj, g_sms_point);
    lms_avail = calc_lms(obj, g_lms_point);
    mms_avail = calc_mms(obj, g_mms_point);
    ...
    if (getMsgType() == "sms")
    {
        ...
        if ((count * npages) > sms_avail)
        {
            message = getLang('msg_not_enough_point') + "\n"
                    + getLang('available_sms_number') + sms_avail  + "\n"
                    + getLang('arranged_sms_number') + (count * npages);
            alert(message);
            return false;
...
function display_bytes() {
    ...
    for (var i = 0; i < size; i++) {
        ...
        $textarea = jQuery('.phonescreen', li);
        var bytes = getTextBytes($textarea.val())[0];
...
function refreshCallbackList() {
    var params = new Array();
    var response_tags = new Array('error','message','data');
    exec_xml('mobilemessage', 'getMobilemessageCallbackNumbers', params, completeGetCallbackList, response_tags);
}
...
function deleteCallback(callback_srl) {
    ...
    var response_tags = new Array('error','message');
    exec_xml('mobilemessage', 'procMobilemessageCallbackNumberDelete', params, function() { refreshCallbackList(); }, response_tags);
}
...
function request_default_number(phonenum) {
    ...
    var response_tags = new Array('error','message');
    exec_xml('mobilemessage', 'procMobilemessageSetDefaultCallbackNumber', params, function() { refreshCallbackList(); }, response_tags);
}
...
function pb_keep_message_content(content) {
    ...
    var response_tags = new Array('error','message');
    exec_xml('mobilemessage','procMobilemessageKeepContent', params, function() { alert('내용을 저장하였습니다'); }, response_tags);
}
...
function pb_search_list(search_word) {
    ...
    exec_xml('mobilemessage','getMobilemessagePurplebookSearch', params, completePurplebookSearch, response_tags);
}
...
function completeLoadSavedMessages(ret_obj,response_tags) {
    ...
    if (ret_obj['data']) {
        ...
        for (var i = 0; i < data.length; i++) {
            $list.append('<li title="' + data[i].content + '"><span class="content">' + data[i].content.substring(0,16) + '</span><span class="delete" keeping_srl="'+data[i].keeping_srl+'"></span></li>');
        }
    } else {
...
function pb_load_saved_messages() {
    var params = new Array();
    var response_tags = new Array('error','message','data');
    exec_xml('mobilemessage','getMobilemessageSavedMessages', params, completeLoadSavedMessages, response_tags);
}

function pb_load_recent_numbers() {
    var params = new Array();
    var response_tags = new Array('error','message','data');
    exec_xml('mobilemessage','getMobilemessageLatestNumbers', params, completeLoadRecentNumbers, response_tags);
}
...
function pb_load_list(node) {
    ...
    jQuery.ajax({
        ...
        , data : { 
                    module : "mobilemessage"
                    , act : "getMobilemessagePurplebookList"
                    , node_id : req_node_id
                    , node_type : '2'
                 }
        , dataType : "json"
        , success : function (data) {
            ...
            jQuery('#btnPurplebookExcelDownload').attr('href', data.base_url + '?module=mobilemessage&act=dispMobilemessagePurplebookDownload&node_type=2&node_id=' + req_node_id);
...
function purplebook_move_node(node_id, dest_id) {
    jQuery.ajax({
        ...
        data : { 
            module : "mobilemessage"
            , act : "procMobilemessagePurplebookMoveNode"
            , node_id : node_id
            , parent_id : dest_id
        },
        success : function (r) {
...
function init_purplebook_tree(img_base)
{
        ...
        "search" : {
            "ajax" : {
                ...
                "data" : function (str) {
                    return { 
                        module : "mobilemessage"
                        , act : "getMobilemessagePurplebookSearchFolder"
                        , search : str
                    }; 
                },
                "success" : function(d) { 
    ...
    .bind("create.jstree", function (e, data) {
        ...
        jQuery.ajax({
            ...
            data : { 
                module : "mobilemessage"
                , act : "procMobilemessagePurplebookAddNode"
    ...
    .bind("rename.jstree", function (e, data) {
        ...
        jQuery.ajax({
            ...
            data : { 
                module : "mobilemessage"
                , act : "procMobilemessagePurplebookRenameNode"
                , node_id : node_id
                , node_name : node_name
            }, 
            success : function(r) {
...
function get_cashinfo()
{
    ...
    exec_xml('mobilemessage', 'getMobilemessageCashInfo', params, completeGetCashInfo, response_tags);
}
...
function get_pointinfo()
{
    ...
    exec_xml('mobilemessage', 'getMobilemessagePointInfo', params, completeGetPointInfo, response_tags);
}
...
(function($) {
     ...
     **/
 	function addNum(newNum, rName) {
		...
		{
            /*
            if (!jQuery.data(document.body,'dup'+newNum)) {
                jQuery.data(document.body,'dup'+newNum,newNum);
            }
            */
	...
	{ 
        ...
        var response_tags = new Array('error','message');
        exec_xml('mobilemessage', 'procMobilemessageInsertRecentReceiver', params, function() { }, response_tags);
    ...
    function add_bulklist_to_purplebook()
    {
        ...
        text = $('#smsPurplebookBulkList').val();
        if (text == "") {
            return 0;
        }
    ...
    */
    ...
    jQuery(function($) {
        ...
        // draggable layer
        $('[class^=layer_].draggable').draggable();

        // tipsy
        $('input, a, img, button').tipsy();
        ...
        $('.delete','#smsPurplebook .layer_recent').live('click',function() {
            ...
            var response_tags = new Array('error','message');
            exec_xml('mobilemessage', 'procMobilemessageDeleteRecentNumber', params, function(ret_obj,response_tags) { pb_load_recent_numbers(); }, response_tags);
        });

        // delete recent content
        $('.delete','#smsMessage .layer_messages').live('click',function() {
            var keeping_srl = $(this).attr('keeping_srl');
            var params = new Array();
            params['keeping_srl'] = keeping_srl;
            var response_tags = new Array('error','message');
            exec_xml('mobilemessage', 'procMobilemessageDeleteRecentContent', params, function(ret_obj,response_tags) { pb_load_saved_messages(); }, response_tags);
        });
        ...
        $('#smsPurplebookDoMove').click(function() {
            ...
            */
            ...
            $.ajax({
                ...
                , data : { 
                            module : "mobilemessage"
                            , act : "procMobilemessagePurplebookMoveList"
        ...
        $('#smsPurplebookDoCopy').click(function() {
            ...
            */
            ...
            $.ajax({
                ...
                , data : { 
                            module : "mobilemessage"
                            , act : "procMobilemessagePurplebookCopy"
                            , node_list : JSON.stringify(list)
                            , node_id : node_id
                         }
                , dataType : "json"
        ...
        $('#btn_append_id','#smsPurplebook').click(function() {
            ...
            exec_xml('mobilemessage', 'procMobilemessagePurplebookShareNode', params, completeShareNode, response_tags);
        });
        ...
        $('.delete','#smsPurplebook #share_list').live('click',function() {
            ...
            exec_xml('mobilemessage', 'procMobilemessagePurplebookUnshareNode', params, function(ret_obj,response_tags) { completeUnshareNode(node_id,ret_obj,response_tags) }, response_tags);
        ...
        */
        ...
        $('.phonescreen','#smsPurplebookContentInput').live('click', function(event) { 
            ...
            /*
            $('.phonescreen','#smsPurplebookContentInput').removeClass('on');
            $(this).addClass('on');
            */
        });
        // saver cursor position
        ...
        */
        ...
        $('#btnMembers').click(function() {
            ...
            $('#addRecipients').attr('title', '왼쪽에 체크된 그룹의 멤버들을 추가합니다.');
        ...
        */
        ...
        /*
        $('#smsPurplebookAccordian').toggle(
            function() {
                $(this).attr('mode','folder');
                $treeView = $('.left .treeView', $(this).parent());
                $right = $('.right', $(this).parent());
                $right.attr('back_width', $right.width());
                $treeView.width($treeView.width() + $right.width());
                $right.css('display','none');
                $(this).addClass('leftArrow');
                return false;
            }
            , function() {
                $(this).attr('mode','address');
                $treeView = $('.left .treeView', $(this).parent());
                $right = $('.right', $(this).parent());
                $right.css('display','block');
                $treeView.css('width', $treeView.width() - $right.attr('back_width'));
                $right.width($right.attr('back_width'));
        ...
        */
        ...
        $('#btnPurplebook').click(function() {
            ...
            $('#smsPurplebookListPane').css('visibility', 'visible');
            $('#smsPurplebookListPane').css('width', '60%');
            $('.smsPurplebook .smsPurplebookTreePane').css('width', '40%');
            $('#addGroupToRecipient').attr('title', '왼쪽에 체크된 폴더의 명단을 추가합니다.');
            $('#addMemberToRecipient').attr('title', '왼쪽에 체크된 명단을 추가합니다.');
            $('#smsPurplebookTree').jstree("check_all");
            return false;
        });

        $('#btnPurplebookFolder').click(function() {
            $(this).addClass('purplebookOn');
            $('#smsOrganization').css('display', 'none');
            $('#smsPurplebookListPane').css('visibility', 'hidden');
            $('.smsPurplebook .smsPurplebookTreePane').css('width', '100%');
            $('#smsPurplebookListPane').css('width', '0%');
            return false;
        });
        */
        ...
        */
        ...
        $('#btn_delDup').click(function() {
            ...
            setTimeout(function() {
                var count=0;
                $('li', '#smsPurplebookTargetList').each( function(i) {
        ...
        */
        ...
        $('.btn_closex', '#smsPurplebook,#smsMessage').click(function() {
            show_and_hide($(this).parent().parent());
            return false;
        });
        ...
        $('#smsPurplebookButtonAddCallback').click(function() {
            ...
            var response_tags = new Array('error','message');
            exec_xml('mobilemessage', 'procMobilemessageCallbackNumberInsert', params, function() {
               refreshCallbackList();
        ...
        */
        ...
        $('.btn_clear','#smsPurplebookContentInput').live('click', function() {
            $('textarea',$(this).parent().parent()).val('').focus();
            return false;
        });
        ...
        $('#btn_pop_manual','#smsPurplebook').click(function() {
            $obj = $('.layer_menual','#smsPurplebook');
            show_and_hide($obj,null,{show_func:function(){
                var manual_url = 'http://www.coolsms.co.kr/?mid=purplebook_manual';
                if (g_manual_mid.length > 0) manual_url = current_url.setQuery('mid',g_manual_mid)
                if(!$obj.attr('first_show')) {
                    $('.bodyArea','#smsPurplebook .layer_menual').html('<iframe src="' + manual_url + '" frameborder="0" style="border:0 none; width:100%; height:100%; padding:0; margin:0;"></iframe>');
        ...
        */
        ...
        $('.inputTypeText','#smsPurplebook').focus(function() {
            $(this).addClass('on');
            //if(this.className == 'loginId') {this.className='loginIdOn';} this.focus();
        });
        ...
        $('.btn_copy','#smsPurplebook .layer_addrbook').click(function() {
            ...
            $.ajax({
                ...
                , data : { 
                            module : "mobilemessage"
                            , act : "procMobilemessagePurplebookAddList"
                            , parent_node : node_id
                            , data : JSON.stringify(list)
                         }
                , dataType : "json"
