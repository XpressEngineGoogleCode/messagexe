<?php
    ...
    class purplebookModel extends purplebook {
        ...
        function getMobilemessageList() {
            $query_id = 'purplebook.getMobilemessageList';
        ...
        function getMessageInfo($args) {
            $query_id = 'purplebook.getMobilemessage';
            return executeQuery($query_id, $args);
        }
        ...
        function getMessagesInGroup($args) {
            $query_id = 'purplebook.getMobilemessagesInGroup';
        ...
        function getMessagesGrouping($args) {
            ...
            if (strtolower(substr($db_info->db_type, 0, 5)) == 'mysql')
                $query_id = 'purplebook.getMobilemessageGrouping_MySQL';
            else
                $query_id = 'purplebook.getMobilemessageGrouping';
            ...
            if (strtolower(substr($db_info->db_type, 0, 5)) != 'mysql') {
                foreach ($output->data as $no => $row) {
                    unset($args);
                    $args->gid = $row->gid;
                    $msginfo = executeQueryArray('purplebook.getMobilemessageGroupMsgInfo', $args);
        ...
        function getUserIDsByPhoneNumber($phone_num, $country_code='82') {
            ...
            $query_id = 'purplebook.getMapping';
            $output = executeQueryArray($query_id, $args);
        ...
        {
            $query_id = 'purplebook.getValCode';
            $args = new StdClass();
        ...
        function isProhibitedNumber($phonenum, $country='82') {
            $query_id = 'purplebook.getProhibit';
            $args = new StdClass();
        ...
        function getNotiDocInfos($module_srl) {
            ...
            $query_id = "purplebook.getNotiDocInfoByModuleSrl";
        ...
        function getNotiComInfo($module_srl) {
            ...
            $query_id = "purplebook.getNotiComInfoByModuleSrl";
            $args->module_srl = $module_srl;
            $output = executeQuery($query_id, $args);
            if (!$output->toBool() || !$output->data) return false;
