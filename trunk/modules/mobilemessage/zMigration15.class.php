<?php
    ...
    class zMigration {
        ...
        function dbConnect() {
            switch($this->db_info->master_db["db_type"]) {
                case 'mysql' :
                case 'mysql_innodb' :
                        $this->connect =  @mysql_connect($this->db_info->master_db["db_hostname"], $this->db_info->master_db["db_userid"], $this->db_info->master_db["db_password"]);
                        if(!mysql_error()) @mysql_select_db($this->db_info->master_db["db_database"], $this->connect);
                        if(mysql_error()) return mysql_error();
                        if($this->source_charset == 'UTF-8') mysql_query("set names 'utf8'");
                    break;
                case 'cubrid' :
                        $this->connect = @cubrid_connect($this->db_info->master_db["db_hostname"], $this->db_info->master_db["db_port"], $this->db_info->master_db["db_database"], $this->db_info->master_db["userid"], $this->db_info->master_db["password"]);
                        if(!$this->connect) return 'database connect fail';
                    break;
                case 'sqlite3_pdo' :
                        if(substr($this->db_info->master_db["db_database"],0,1)!='/') $this->db_info->master_db["db_database"] = $this->path.'/'.$this->db_info->master_db["db_database"];
                        if(!file_exists($this->db_info->master_db["db_database"])) return "database file not found";
                        $this->handler = new PDO('sqlite:'.$this->db_info->master_db["db_database"]);
                        if(!file_exists($this->db_info->master_db["db_database"]) || $error) return 'permission denied to access database';
                    break;
                case 'sqlite' :
                        if(substr($this->db_info->master_db["db_database"],0,1)!='/') $this->db_info->master_db["db_database"] = $this->path.'/'.$this->db_info->master_db["db_database"];
                        if(!file_exists($this->db_info->master_db["db_database"])) return "database file not found";
                        $this->connect = @sqlite_open($this->db_info->master_db["db_database"], 0666, $error);
