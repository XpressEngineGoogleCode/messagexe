<?php
    ...
    if (in_array(Context::get('act'), array('dispMemberSignUpForm', 'dispMemberModifyInfo')) && $called_position == 'before_display_content') {
        // loading lang-file
        Context::loadLang('./addons/authentication/lang');
        ...
        // Class
        require_once($addon_path.'authentication.class.php');
        $validator = new AuthenticationAddon($addon_info);
        ...
        if ($addon_info->global=='Y') {
            ...
            Context::addJsFile('./addons/authentication/tpl/js/data.js');
...
<script language=javascript>
    (function($) {
        jQuery(function($) {
            ...
            flag.onchange = function() { this.form.{$config->countrycode_fieldname}.value = flag.options[flag.options.selectedIndex].value; }
        ...
        $url = getSiteUrl().'?module=authentication&act=dispAuthenticationValidation';
        $buttonUrl = getSiteUrl().'addons/authentication/tpl/img/button.gif';
        Context::addHtmlHeader("
<script language=javascript>
    (function($) {
        jQuery(function($) {
            ...
            if ('{$addon_info->hide_checkbox}' == 'Y') {
                ...
                $('#open_{$config->validationcode_fieldname}').parent().css('display', 'none');
            }
    ...
    /**
     * @brief authentication 모듈의 dispAuthenticationValidation 기능을 옮겨옴.
     *
     * 애드온 작동 액션: dispAuthenticationValidation
     * 애드온 작동 시점: before_display_content
     */
    if (Context::get('act') == 'dispAuthenticationValidation' && $called_position == 'after_module_proc') {
        // 언어파일 로딩
        Context::loadLang('./addons/authentication/lang');
        $lang = Context::get('lang');

        // 템플릿 PATH
        $this->setTemplatePath('addons/authentication/tpl');

        // Class
        require_once($addon_path.'authentication.class.php');
        $validator = new Authentication($addon_info);
        ...
        $this->setLayoutPath('addons/authentication/tpl');
        ...
        if (Context::get('cellphone_number1')) {
            ...
            // 금지번호 체크
            if ($oAuthenticationModel->isProhibitedNumber($args->callno, $args->country)) {
            ...
            if ($addon_info->unique && $addon_info->unique == 'Y') {
                $userids = $oAuthenticationModel->getUserIDsByPhoneNumber($args->callno, $args->country);
    ...
    if (in_array(Context::get('act'), array('procMemberInsert', 'procMemberModifyInfo')) && $called_position == 'before_module_proc') {
        ...
        $oAuthenticationController = &getController('authentication');
        $oAuthenticationModel = &getModel('authentication');

        // 기본모듈 미설치 오류
        if (!$oAuthenticationController || !$oAuthenticationModel) {
        ...
        if (Context::get('act') == 'procMemberModifyInfo') {
            ...
            $args->country = $country;
            $oAuthenticationController->insertMapping($args);
        }

        // 금지번호 체크
        if ($phonenumber && $oAuthenticationModel->isProhibitedNumber($phonenumber, $country)) {
        ...
        if ($phonenumber && $addon_info->unique && $addon_info->unique == 'Y') {
            $userids = $oAuthenticationModel->getUserIDsByPhoneNumber($phonenumber, $country);
            $is_unique = true;
            if (Context::get('act') == 'procMemberModifyInfo') {
                $logged_info = Context::get('logged_info');
